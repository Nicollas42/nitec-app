...
export const useProdutosStore = defineStore('produtos_store', () => {
    ...
    const buscar_produtos = async (forcar_atualizacao = false) => {
        ...
        try {
            ...
            if (produtos_recebidos.length === 0 && lista_produtos.value.length > 0) {
                console.warn('[produtos_store] Resposta vazia ignorada — mantendo dados persistidos.');
                return;
            }
    ...
    return { lista_produtos, modo_offline, ultima_atualizacao, buscar_produtos };
}, {
    persist: {
