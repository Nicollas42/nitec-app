...
export function useLogicaPdv() {
    ...
    const alterar_quantidade_db = (id_item_comanda, acao) => {
        ...
        acoes_pendentes_db.value.push({ tipo: 'alterar', id: id_item_comanda, acao });
    };
    ...
    const remover_item_db = (id_item_comanda) => {
        ...
        acoes_pendentes_db.value.push({ tipo: 'remover', id: id_item_comanda });
    };

    // 🟢 ITENS NOVOS DO CARRINHO (Venda Balcão ou Lançamento)
    ...
    const alterar_quantidade_novo = (idx, acao) => {
        if (acao === 'incrementar') {
            carrinho_venda.value[idx].quantidade++;
        } else if (acao === 'decrementar') {
            if (carrinho_venda.value[idx].quantidade > 1) {
                carrinho_venda.value[idx].quantidade--;
            } else {
    ...
    const sincronizar_pendencias_bd = async () => {
        for (const tarefa of acoes_pendentes_db.value) {
            if (tarefa.tipo === 'alterar') {
                await api_cliente.post(`/alterar-quantidade-item/${tarefa.id}`, { acao: tarefa.acao });
            } else if (tarefa.tipo === 'remover') {
                await api_cliente.delete(`/remover-item-comanda/${tarefa.id}`);
            }
        }
        acoes_pendentes_db.value = []; 
    };
    ...
    const processar_acao_principal = async () => {
        ...
        if (id_comanda_pagamento.value) {
            try {
                ...
                if (carrinho_venda.value.length > 0) {
                    ...
                    await api_cliente.post(`/adicionar-itens-comanda/${id_comanda_pagamento.value}`, payload);
                }
        ...
        if (id_comanda_vinculada.value) {
            try {
                ...
                roteador.go(-1);
            } catch (e) { toast_global.exibir_toast("Erro ao atualizar a comanda.", "erro"); }
            finally { processando_finalizacao.value = false; }
        } else {
            try {
    ...
    return {
        ...
        adicionar_ao_carrinho, remover_do_carrinho, alterar_quantidade_novo, // 🟢 Exportado aqui!
