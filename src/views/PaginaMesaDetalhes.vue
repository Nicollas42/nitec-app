<template>
    <div class="tela_detalhes_mesa p-6 bg-gray-100 min-h-screen font-sans flex flex-col relative">
        
        <div v-if="estado_carregamento" class="carregamento flex-1 flex items-center justify-center">
            <p class="texto_carregando text-xl font-bold text-gray-500 animate-pulse">Carregando dados da mesa...</p>
        </div>

        <div v-else-if="dados_mesa" class="conteudo_mesa flex flex-col h-full">
            
            <header class="cabecalho_mesa flex justify-between items-center bg-white p-6 rounded-2xl shadow-sm border border-gray-200 mb-6">
                ...
                <div class="acoes_topo flex gap-3">
                    <button @click="adicionar_novo_cliente" class="botao_add_cliente bg-blue-100 hover:bg-blue-600 hover:text-white text-blue-700 font-black px-6 py-3 rounded-xl transition-colors shadow-sm border border-blue-200 uppercase text-sm">
                        + Sub-Comanda (Novo Cliente)
                    </button>
                    <button @click="voltar_mapa" class="botao_voltar bg-gray-800 hover:bg-black text-white px-8 py-3 rounded-xl font-bold transition-all shadow-md">
                        ⬅ Voltar
                    </button>
                </div>
            </header>

            <main class="area_comandas flex-1 grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-6 items-start">
                ...
                >
                    <div class="cabecalho_conta flex justify-between items-start mb-6 border-b pb-4">
                        <div>
                            ...
                            <h2 class="nome_dono text-xl font-black text-gray-800 uppercase mt-1">
                                {{ index === 0 ? 'Conta Geral da Mesa' : 'Conta Individual' }}
                            </h2>
                    ...
                    <button @click="abrir_pdv_para_comanda(comanda.id)" class="botao_lancar_pdv w-full bg-blue-500 hover:bg-blue-600 text-white font-black py-4 rounded-xl shadow-md transition-transform transform active:scale-95 uppercase tracking-widest mb-6 flex items-center justify-center gap-2">
                    ...
                    <button @click="fechar_conta_comanda(comanda.id)" class="botao_fechar_conta w-full bg-green-500 hover:bg-green-600 text-white font-black py-4 rounded-xl shadow-md transition-all uppercase tracking-widest">
                        💸 Fechar Conta
                    </button>
                </article>
            </main>
        ...
        <div v-if="modal_cliente_visivel" class="modal_backdrop fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50">
...
const { 
    dados_mesa, estado_carregamento, voltar_mapa, abrir_pdv_para_comanda, 
