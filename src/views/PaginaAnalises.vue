    ...
    <div class="p-6 bg-[var(--bg-page)] h-full overflow-y-auto font-sans relative transition-colors duration-300">
        ...
        <header class="flex flex-col lg:flex-row justify-between items-start lg:items-center mb-6 gap-4 bg-[var(--bg-card)] p-6 rounded-3xl shadow-sm border border-[var(--border-subtle)] transition-colors duration-300">
            <div>
                <div class="flex items-center gap-3">
                    <h1 class="text-2xl font-black text-[var(--text-primary)] tracking-tight">Inteligência de Negócios</h1>
                    <button @click="modal_config_visivel = true" class="text-[var(--text-muted)] hover:text-nitec_blue transition-colors" title="Configurar Horário do Turno">
            ...
            <div class="flex flex-wrap items-center gap-2 w-full lg:w-auto mt-4 lg:mt-0">
                ...
                <button @click="buscar_dados" class="bg-nitec_blue hover:bg-blue-700 text-white h-[38px] w-[38px] rounded-xl shadow-md transition-all flex items-center justify-center active:scale-95 flex-shrink-0" title="Pesquisar Período">🔍</button>
            </div>
        </header>
        ...
        <div v-else-if="dados_dashboard" class="animate-in fade-in duration-500 pb-20">
            ...
            <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4 mb-8">
                ...
                <div class="bg-[var(--bg-card)] p-6 rounded-3xl border border-[var(--border-subtle)] shadow-sm hover:bg-[var(--bg-card-hover)] transition-all group flex flex-col">
                    <div class="flex justify-between items-center mb-3">
                        <span class="text-[10px] font-bold text-[var(--text-muted)] uppercase tracking-widest">Vendas Avulsas (Balcão)</span>
                ...
                <div class="bg-[var(--bg-card)] p-6 rounded-3xl border border-[var(--border-subtle)] shadow-sm hover:bg-[var(--bg-card-hover)] transition-all group flex flex-col">
                    <div class="flex justify-between items-center mb-3">
                        <span class="text-[10px] font-bold text-[var(--text-muted)] uppercase tracking-widest">Ticket Médio</span>
            ...
            <nav class="flex gap-1 mb-8 bg-[var(--bg-card)] border border-[var(--border-subtle)] p-1.5 rounded-2xl w-fit overflow-x-auto max-w-full shadow-inner">
                <button v-for="aba in ['estatísticas', 'estoque S/ giro', 'equipe', 'auditoria']" :key="aba"
                    @click="aba_ativa = aba"
                    :class="aba_ativa === aba ? 'bg-[var(--bg-page)] border border-[var(--border-subtle)] shadow-sm text-nitec_blue font-black' : 'text-[var(--text-muted)] hover:text-[var(--text-primary)] font-bold'"
        ...
        <div v-if="modal_config_visivel" class="fixed inset-0 bg-black/80 backdrop-blur-sm z-[100] flex items-center justify-center p-4">
            <div class="bg-[var(--bg-card)] rounded-[2.5rem] p-8 max-w-sm w-full shadow-2xl flex flex-col text-center border-2 border-[var(--border-subtle)]">
                ...
                <div class="text-left mb-8">
                    <label class="block text-[10px] font-black text-[var(--text-muted)] uppercase tracking-widest mb-2 pl-2">Horário de Virada</label>
                ...
                <header class="p-8 bg-[var(--bg-page)] border-b border-[var(--border-subtle)] flex justify-between items-start">
                    <div>
                        ...
                        <div class="text-[10px] font-black uppercase tracking-widest mt-2 flex flex-wrap items-center gap-1.5">
                            ...
                            <span class="text-[var(--text-muted)]">{{ comanda_selecionada.buscar_mesa ? comanda_selecionada.buscar_mesa.nome_mesa : 'BALCÃO' }}</span>
                            <span class="text-[var(--text-muted)]">•</span>
                ...
                <div class="p-8 flex-1 overflow-y-auto max-h-[50vh]">
                    <div class="space-y-4">
                        ...
                        <div v-for="item in comanda_selecionada.listar_itens" :key="item.id" class="flex justify-between items-center">
                            <p class="text-xs font-bold text-[var(--text-primary)] uppercase tracking-tight"><span class="bg-[var(--bg-page)] border border-[var(--border-subtle)] text-[var(--text-muted)] px-2 py-0.5 rounded text-[10px] mr-2">{{ item.quantidade }}x</span>{{ item.buscar_produto?.nome_produto || 'Produto Removido' }}</p>
