<template>
    <div class="bg-[var(--bg-card)] rounded-[2rem] shadow-sm border border-[var(--border-subtle)] flex flex-col h-[70vh] transition-colors duration-300">
        <div class="p-6 border-b border-[var(--border-subtle)] flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-[var(--bg-page)] rounded-t-[2rem] shrink-0 transition-colors duration-300">
            <div>
                <h2 class="text-lg font-black text-[var(--text-primary)] tracking-tight">Rastreio de Operacoes</h2>
            ...
            <table class="w-full text-left text-xs">
                <thead class="bg-[var(--bg-page)] text-[var(--text-muted)] sticky top-0 z-10 border-b border-[var(--border-subtle)] shadow-sm whitespace-nowrap transition-colors duration-300">
                    <tr>
                        ...
                        <th class="py-2.5 px-6 font-black uppercase tracking-widest text-[9px] w-28">Operacao</th>
                        <th class="py-2.5 px-6 font-black uppercase tracking-widest text-[9px] w-36">Responsavel</th>
                        <th class="py-2.5 px-6 font-black uppercase tracking-widest text-[9px]">Acao Executada / Detalhes</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-[var(--border-subtle)] bg-[var(--bg-card)] transition-colors duration-300">
                    ...
                    <tr
                        v-for="(evento, idx) in log_exibido"
                        :key="idx"
                        class="transition-colors group"
                        :class="ehEntradaForcada(evento) ? 'bg-violet-500/5 hover:bg-violet-500/10' : 'hover:bg-[var(--bg-card-hover)]'"
                    >
                        <td class="py-4 px-6 text-[11px] font-black text-[var(--text-muted)] whitespace-nowrap">{{ formatarDataLog(evento.data_hora) }}</td>

                        <td class="py-4 px-6 whitespace-nowrap">
                            <span
                                class="px-2 py-1.5 rounded-md font-black uppercase tracking-widest text-[9px]"
                                :class="{
                                    'bg-violet-500/15 text-violet-600 border border-violet-500/30': ehEntradaForcada(evento),
                                    'bg-blue-500/10 text-blue-500 border border-blue-500/20': evento.tipo_evento === 'entrada' && !ehEntradaForcada(evento),
                            ...
                            <div class="flex items-start justify-between gap-4">
                                <div class="flex flex-col gap-1.5">
                                    ...
                                    <div class="flex items-center gap-2 flex-wrap mt-1">
                                        <span
                                            v-if="evento.detalhes_extras"
                                            class="inline-block w-fit px-2 py-0.5 rounded text-[10px] font-bold border"
                                            :class="ehEntradaForcada(evento)
                                                ? 'bg-violet-500/10 text-violet-700 border-violet-500/30'
                                                : 'bg-[var(--bg-page)] text-[var(--text-muted)] border-[var(--border-subtle)]'"
                                        >
                                            {{ formatarDetalhesExtras(evento) }}
                                        </span>
                                        <span v-if="ehEntradaForcada(evento)" class="inline-block w-fit px-2 py-0.5 rounded bg-violet-500/15 text-violet-700 text-[10px] font-black border border-violet-500/30 uppercase tracking-widest">
                                            Entrada Forcada
                                        </span>
...
const ehEntradaForcada = (evento) => {
    const texto = `${evento?.titulo || ''} ${evento?.descricao || ''} ${evento?.detalhes_extras || ''}`.toLowerCase();
    return evento?.tipo_evento === 'entrada' && texto.includes('entrada forcada via pdv');
};

const formatarDetalhesExtras = (evento) => {
    if (!evento?.detalhes_extras) return '';
    if (evento.tipo_evento === 'entrada') return `Fornecedor: ${evento.detalhes_extras}`;
    return evento.detalhes_extras;
};
