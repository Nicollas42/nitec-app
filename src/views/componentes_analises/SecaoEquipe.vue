<template>
    <div class="bg-[var(--bg-card)] rounded-3xl shadow-sm border border-[var(--border-subtle)] flex flex-col relative transition-colors duration-300 mb-10">
            ...
            <table class="w-full text-left text-sm whitespace-nowrap">
                <thead class="bg-[var(--bg-card)] text-[var(--text-muted)] border-b border-[var(--border-subtle)] transition-colors duration-300">
                    <tr>
                        ...
                        <th class="py-4 px-6 font-bold uppercase text-[9px] tracking-widest text-center">Atendimentos</th>
                ...
                <tbody class="divide-y divide-[var(--border-subtle)] transition-colors duration-300">
                    <tr v-for="(garcom, index) in equipe_filtrada" :key="garcom.name" class="hover:bg-[var(--bg-card-hover)] transition-colors group">
                        ...
                        <td class="py-4 px-6 text-center">
                            <div class="flex flex-col">
                                <span class="font-bold text-[var(--text-primary)]">{{ garcom.total_mesas }} mesas</span>
                                <span class="text-[9px] text-[var(--text-muted)] font-bold uppercase tracking-widest mt-0.5">~{{ garcom.tempo_medio_minutos }} min/mesa</span>
                            </div>
                        </td>
                    ...
                    <tr v-if="!equipe_filtrada || equipe_filtrada.length === 0">
                        <td colspan="6" class="p-10 text-center text-[10px] tracking-widest uppercase text-[var(--text-muted)] font-black italic">Nenhum membro da equipa atende a estes critérios de pesquisa.</td>
                    </tr>
                </tbody>
            </table>
        </div>
