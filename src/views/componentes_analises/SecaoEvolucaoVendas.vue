<template>
    <div class="flex flex-col relative w-full mb-10">
        <div class="flex justify-between items-end mb-2 px-2">
            <span class="text-[10px] font-black text-gray-400 uppercase tracking-widest ml-2">Saúde Financeira</span>
        ...
        <div v-show="visivel" class="bg-white rounded-3xl shadow-sm border border-gray-100 flex flex-col relative transition-all">
            <div class="p-6 bg-gray-50 border-b border-gray-100 flex items-center gap-2 rounded-t-3xl">
                <h2 class="text-sm font-black text-gray-800 uppercase">Evolução de Faturamento</h2>
            ...
            <div class="p-6">
                <div class="h-72 w-full bg-white rounded-2xl">
                    <Line v-if="dados_grafico" :data="dados_grafico" :options="opcoes_grafico" />
                    <div v-else class="h-full flex items-center justify-center text-xs text-gray-400 italic">Sem dados suficientes para gerar a curva.</div>
                </div>
            </div>
        </div>
    </div>
