<template>
    <div class="flex flex-col relative w-full">
        <div class="flex justify-between items-end mb-2 px-2">
            <span class="text-[10px] font-black text-[var(--text-muted)] uppercase tracking-widest ml-2">Comportamento</span>
        ...
        <div v-show="visivel" class="bg-[var(--bg-card)] rounded-3xl shadow-sm border border-[var(--border-subtle)] flex flex-col relative transition-all duration-300">
            <div class="p-6 bg-[var(--bg-page)] border-b border-[var(--border-subtle)] flex items-center gap-2 rounded-t-3xl transition-colors duration-300">
                <h2 class="text-sm font-black text-[var(--text-primary)] uppercase">Faturamento por Dia da Semana</h2>
            ...
            <div class="p-6">
                <div class="h-64 w-full bg-[var(--bg-page)] rounded-2xl border border-[var(--border-subtle)] p-2 transition-colors duration-300">
                    ...
                    <div v-else class="h-full flex items-center justify-center text-xs text-[var(--text-muted)] italic">Sem dados suficientes.</div>
                </div>
            </div>
        </div>
    </div>
