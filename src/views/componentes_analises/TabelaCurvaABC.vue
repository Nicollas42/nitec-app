<template>
    <div class="flex flex-col relative w-full">
        <div class="flex justify-between items-end mb-2 px-2">
            <span class="text-[10px] font-black text-[var(--text-muted)] uppercase tracking-widest ml-2">Painel de Lucro</span>
        ...
        <div v-show="visivel" class="bg-[var(--bg-card)] rounded-3xl shadow-sm border border-[var(--border-subtle)] flex flex-col relative transition-all duration-300">
            <div class="p-6 bg-[var(--bg-page)] border-b border-[var(--border-subtle)] flex flex-col md:flex-row justify-between items-start md:items-center gap-4 rounded-t-3xl transition-colors duration-300">
                <div class="flex items-center gap-2">
                    <h2 class="text-sm font-black text-[var(--text-primary)] uppercase">Curva ABC (Lucratividade Real)</h2>
                ...
                <div class="flex items-center gap-2 w-full md:w-auto">
                    ...
                    <button v-if="filtro_produto" @click="filtro_produto = ''" class="text-[10px] text-red-500 hover:text-red-400 font-bold px-3 py-2 bg-red-500/10 rounded-lg transition-colors uppercase tracking-widest border border-red-500/20">
                        Limpar
                    </button>
                </div>
            </div>
            ...
            <div class="overflow-x-auto overflow-y-auto max-h-[400px] rounded-b-3xl custom-scrollbar">
                <table class="w-full text-left text-sm whitespace-nowrap">
                    <thead class="bg-[var(--bg-page)] text-[var(--text-muted)] border-b border-[var(--border-subtle)] sticky top-0 z-10 shadow-sm transition-colors duration-300">
                        <tr>
                            ...
                            <th class="py-4 px-6 font-bold uppercase text-[9px] tracking-wider text-right text-red-500">Custo Total</th>
                            <th class="py-4 px-6 font-bold uppercase text-[9px] tracking-wider text-right text-green-500">Lucro Estimado</th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-[var(--border-subtle)] transition-colors duration-300">
                        <tr v-for="(p, index) in produtos_filtrados" :key="p.produto_id" class="hover:bg-[var(--bg-card-hover)] transition-colors">
                            ...
                            <td class="py-4 px-6 text-center font-bold text-[var(--text-muted)]">{{ p.quantidade_total }}x</td>
                            <td class="py-4 px-6 text-right font-bold text-[var(--text-muted)]">R$ {{ Number(p.receita_total).toFixed(2) }}</td>
                            <td class="py-4 px-6 text-right font-bold text-red-500">R$ {{ (Number(p.receita_total) - Number(p.lucro_total)).toFixed(2) }}</td>
                            <td class="py-4 px-6 text-right font-black text-green-500">R$ {{ Number(p.lucro_total).toFixed(2) }}</td>
                        </tr>
                        <tr v-if="!produtos_filtrados || produtos_filtrados.length === 0">
                            <td colspan="5" class="p-8 text-center text-xs text-[var(--text-muted)] font-medium italic">Nenhum produto encontrado.</td>
                        </tr>
                    </tbody>
                </table>
            </div>
